...
class WallStopAccelTest(unittest.TestCase):
    ...
    def test_io(self):
        ...
        time.sleep(5.0)
        left, right = self.set_and_get(0,0,0,9) #total : 49
        self.assertTrue(2000 < left == right, "can't accerelate")

        left, right = self.set_and_get(15,0,20,15) #total : 50
        self.assertTrue(left == right == 0, "cant'stop again")

if __name__ == '__main__':
    time.sleep(3)
    rospy.init_node('test_wall_stop_accel')
    rostest.rosrun('pimouse_run_2025','test_wall_stop_accel',WallStopAccelTest)
